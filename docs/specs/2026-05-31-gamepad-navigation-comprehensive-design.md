...
| 期4 手势等价动作 | 部分已有 | `GamepadLongPressActions` 已覆盖历史/集合/搜索历史等条目；标签页有 X 删除；字体/词典/同步 URL/音频源/Local Audio 等重排列表有上移/下移按钮，`gamepad_reorder_test.dart` 覆盖索引补偿；hold-A 输入层已有 `GamepadLongPressIntent`，本轮接入 reader/popup caret 长按路径，并补齐 Android reader 与 Flutter widget `gameButtonA` KeyDown/KeyUp 判定 | 仍需真实手柄复测集合/标签/字体/词典/同步 URL/音频源的长按、删除、重排路径 |
| 期5 阅读器 + WebView 专项 | 部分已有 | 弹窗上下跳项/左右逐字、图片停点、reader 图片 `onImageTap`、R3 振假名、LB/RB 复用滚动/翻页均有代码与 `reader_caret_scripts_test.dart` 结构测试；本轮修复了 disabled 按钮误入停点；全屏插图已消费 LB/RB 翻页和 R3 缩放，见 `illustrations_viewer_page_static_test.dart`；本轮补齐 hold-A → `ReaderCaretScripts.longPressInvocation()`，弹窗 `summary.dict-label` 可通过 `window.__hoshiDictLongPress` 标记词典 | 主页词典 tab 已改用 `DictionaryPopupWebView`，旧 `DictionaryHtmlWidget` 只剩单词典结构化内容页；真实 WebView DOM/边界证据仍需补齐；全屏插图仍需真实手柄复测 |
