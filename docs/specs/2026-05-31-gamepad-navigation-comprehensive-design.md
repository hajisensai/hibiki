...
| 期4 手势等价动作 | 部分已有 | `GamepadLongPressActions` 已覆盖历史/集合/搜索历史等条目；标签页有 X 删除；字体/词典/同步 URL/音频源/Local Audio 等重排列表有上移/下移按钮，`gamepad_reorder_test.dart` 覆盖索引补偿；hold-A 输入层已有 `GamepadLongPressIntent`，本轮接入 reader/popup caret 长按路径，并补齐 Android reader `gameButtonA` KeyDown/KeyUp 判定 | 仍需真实手柄复测集合/标签/字体/词典/同步 URL/音频源的长按、删除、重排路径 |
