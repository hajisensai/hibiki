...
## 4. 冲突即弹：自动同步与立刻同步都弹（用户最终确认）

> 用户最终决定：**自动同步和立刻同步，只要检测到冲突都立即弹解决对话框**（覆盖早期「后台攒着不打断」的选择）。

`_runAutoSync`（开/关书触发，可能正在阅读）与 `runManualFullSync` 遇到冲突维度：

- **先跳过该书该维度的自动解决**：不 import 不 export，不写 base（保持「仍冲突」）。
- 把本轮检出的冲突集合**立即弹出冲突解决对话框**让用户处理。
- 自动同步在后台无页面 context，须经**全局 `navigatorKey`** 弹窗（仓库 LAN 配对 `onPairRequest` 已有此套路，复用）。

### 4.1 防骚扰（必须，否则每次开/关书都弹）

自动同步在每次开/关书触发，冲突未解决前会被反复检出。为避免每翻一次书就弹一次：

1. **单飞**：已有冲突对话框在显示时，后续触发不叠弹（用一个全局 `bool _conflictDialogOpen` 或复用 `syncInProgress` 风格的 notifier 守卫）。
2. **会话级 snooze-on-dismiss**：用户**取消/关闭**某组冲突（未解决）后，本次 app 会话内不再对**同一组冲突**自动弹（用内存 `Set<conflictKey>` 记已忽略集，`conflictKey = assetKey+dimension+local+remote` 版本指纹）。冲突仍可从设置页「N 个冲突待解决」入口手动进入解决。冲突**版本变化**（任一端又更新）或 **app 重启**会清掉 snooze 重新弹。
3. 用户**解决**（选了方向并 Apply）后写 base，冲突消失，自然不再弹。

> 立刻同步是用户主动操作，不受 snooze 影响：手动点「立刻同步」若有冲突总是弹（包括被 snooze 的）。snooze 只压制**自动同步**的重复弹窗。

冲突**不需要单独的持久队列**：任何时刻重算 `SyncBaselines` vs 当前 local/remote 即可得到「当前冲突集合」；snooze 集只在内存、随会话失效。
...
- **自动同步**：检出冲突 → 经全局 `navigatorKey` 弹（受 4.1 单飞 + 会话 snooze 约束）。
...
- **自动同步弹冲突 + 防骚扰**：widget/单测验证 ① 自动同步检出冲突经全局 navigatorKey 弹；② 已有对话框时不叠弹（单飞）；③ 取消后同一冲突指纹本会话不再自动弹（snooze），但冲突指纹变化/手动「立刻同步」仍弹。
...
5. 全局冲突弹窗调度器：单飞守卫 + 会话 snooze 集 + 经 `navigatorKey` 弹（复用 LAN 配对的全局弹窗套路）；`_runAutoSync` 与 `runManualFullSync` 检出冲突后调用它。
