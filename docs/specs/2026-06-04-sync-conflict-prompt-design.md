...
## 4. 冲突即弹：立刻同步立即弹；自动同步等退出书籍再弹（用户最终确认）

> 用户最终决定：**冲突都要弹，但自动同步检出的冲突不打断阅读——等用户退出书籍再弹**；立刻同步是主动操作，立即弹。

### 4.0 关键前提：per-book 自动同步本就不在开书时触发

`sync_auto_trigger.dart` 的触发点只有三个，没有「开书时」：

| 触发 | 时机 | 弹冲突？ |
|---|---|---|
| `triggerAutoSyncAfterClose` | **关书之后**（已退出书籍） | **弹** |
| `triggerAutoSyncOnAppOpen`（`_runAutoSyncAll`） | app 启动，人在书架/首页 | **弹** |
| `triggerAutoSyncOnBackground` | app 切后台（弹窗看不到） | **不弹，攒着** |

因此「等退出书籍再弹」天然成立：**只在「用户不在书里」时弹**。

### 4.1 处理规则

所有同步（自动 + 立刻）遇冲突维度，一律：

- **先跳过该书该维度的自动解决**：不 import 不 export，不写 base（保持「仍冲突」）。
- 是否/何时弹，按下表：

| 来源 | 弹的时机 |
|---|---|
| **立刻同步** `runManualFullSync` | 跑完立即弹（用户主动操作，无论是否在书里——通常本就在设置页） |
| **自动同步·关书后** | 关书完成后弹（已退出书籍） |
| **自动同步·app 启动** | 启动全量同步后、人在书架时弹 |
| **自动同步·切后台** | 不弹；冲突攒着，等下次「关书后 / app 启动」重算时弹 |

- 弹窗须经**全局 `navigatorKey`**（后台同步无页面 context；仓库 LAN 配对 `onPairRequest` 已有此套路，复用）。
- **统一闸门**：调度器弹前检查「当前是否有书籍/阅读页处于活动态」（查 `AppModel` 当前 media 状态）——**在书里就不弹**，把冲突留到退出书籍后的触发点。这条闸门兜住所有路径，确保阅读永不被打断。

### 4.2 防骚扰

1. **单飞**：已有冲突对话框在显示时，后续触发不叠弹（全局 `bool _conflictDialogOpen` 守卫）。
2. **会话级 snooze-on-dismiss**：用户**取消/关闭**某组冲突（未解决）后，本次 app 会话内不再对**同一组冲突**自动弹（内存 `Set<conflictKey>`，`conflictKey = assetKey+dimension+local+remote` 版本指纹）。避免连续关好几本书时反复弹同一冲突。冲突**版本变化**或 **app 重启**清掉 snooze 重新弹。仍可从设置页「N 个冲突待解决」入口手动进入。
3. 用户**解决**（选方向并 Apply）后写 base，冲突消失，自然不再弹。
4. **立刻同步不受 snooze 约束**：手动点若有冲突总是弹（包括被 snooze 的）。snooze 只压制**自动同步**的重复弹窗。

冲突**不需要持久队列**：任何时刻重算 `SyncBaselines` vs 当前 local/remote 即得「当前冲突集合」；snooze 集只在内存、随会话失效。
...
三个入口共用同一个「冲突解决对话框」（compare 的冲突视图）：

- **自动同步**：检出冲突 → 经全局 `navigatorKey` 弹，但受 4.1 时机表（关书后/启动弹、切后台不弹、在书里不弹）+ 4.2 单飞/snooze 约束。
- **立刻同步**：`_syncNow` 跑完 `runManualFullSync` 后若 `conflicts > 0` → 直接弹（用户主动操作，不受 snooze 约束），而不是仅 SnackBar。
- **同步设置页**：新增「N 个冲突待解决」入口/角标（N = 当前重算出的冲突数）；点击手动打开（兜底入口，被 snooze 时也能进）。
...
- **冲突不自动解决**：单测验证 `_runAutoSync`/`runManualFullSync` 遇冲突时不写数据、不写 base（保持「仍冲突」）。
- **自动同步弹冲突时机 + 防骚扰**：widget/单测验证 ① 关书后/app 启动检出冲突经全局 navigatorKey 弹；② **在书里（reader 活动态）不弹**、切后台不弹，留到退出书籍后弹；③ 已有对话框时不叠弹（单飞）；④ 取消后同一冲突指纹本会话不再自动弹（snooze），但冲突指纹变化/手动「立刻同步」仍弹。
- **立刻同步弹冲突对话框**：widget 测试 `_syncNow` 在 `conflicts>0` 时弹出解决对话框（沿用 `tapDeleteAndConfirm` 风格的焦点驱动断言），且不受 snooze 约束。
...
5. 全局冲突弹窗调度器：**在书里不弹**闸门（查 `AppModel` 当前 media）+ 单飞守卫 + 会话 snooze 集 + 经 `navigatorKey` 弹（复用 LAN 配对的全局弹窗套路）；`_runAutoSync`（关书后/app 启动）与 `runManualFullSync` 检出冲突后调用它，切后台触发不弹。
6. `SyncCompareDialog`：`hasConflict` 收紧读 base；Apply 后写 base。
7. 设置页冲突入口/角标 + `_syncNow` 冲突弹窗（手动不受 snooze 约束）。
