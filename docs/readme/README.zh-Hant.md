...
hibiki 是一款多語言沉浸式語言學習工具，把讀 EPUB、聽有聲書、看影片三種輸入收進一套查詞、製卡與統計流程。
...
- **讀 · 聽 · 看，一個 App：** EPUB 閱讀、有聲書同步、影片字幕查詞、Anki 製卡在同一個 App 裡完成，不需要在多個工具之間切換。
...
- **桌面與行動端統一體驗：** Android 與 Windows 共用同一程式碼庫，Material Design 3 自適應佈局，寬螢幕 master-detail、拖放匯入一應俱全。
