...
```text
nonGeneratedDart=243
uiMatched=96
broadUiAdjacentMatched=133
coverageRows=104
gallerySurfaces=84
manifestSurfaces=84
...
- Total non-generated Dart files under `hibiki/lib`: 243
- UI-building files matched by scan: 96
- Broad UI-adjacent files matched by scan: 133
- Entry UI files mapped in `COVERAGE.md`: 3
- Page implementation files mapped in `COVERAGE.md`: 56
- Shared/base/support UI files mapped in `COVERAGE.md`: 45
...
| `reader_quick_settings_sheet.dart` | In-reader quick settings, search, TOC, jump controls, and audiobook/overlay toggles. | `04-reader.svg`, `11-reader-customization.svg` |
...
| `anki_integration.dart` | Anki permission and integration dialogs reached from creator/settings flows. | `07-creator-anki.svg`, `14-profile-language-system.svg` |
| `theme_notifier.dart` | Global Material theme component mapping and MD3 defaults. | `18-component-system.svg`, `05-settings.svg` |
...
| `hibiki_design_tokens.dart` | Shared MD3 spacing, radius, surface, and type tokens. | `18-component-system.svg`, `05-settings.svg` |
| `hibiki_material_components.dart` | Shared MD3 cards, rows, text fields, menus, log panels, popups, search rows, and editor panels. | `18-component-system.svg`, `05-settings.svg` |
...
| `settings_schema.dart` | Schema-defined settings rows and inline controls. | `05-settings.svg`, `18-component-system.svg` |
...
| `sync_compare_dialog.dart` | Sync conflict comparison dialog and choice menu. | `05-settings.svg`, `06-import-and-modals.svg` |
| `sync_settings_schema.dart` | Sync/backup settings schema and WebDAV credential fields. | `05-settings.svg`, `18-component-system.svg` |
...
| `spacing.dart` | Shared spacing extension used across adaptive surfaces. | `18-component-system.svg`, `01-home-navigation.svg` |
...
No new design board is needed for these files. They are not independent screens; they are shared surfaces, schema-driven settings, or helpers whose visual decisions are controlled by the existing component, modal, reader customization, creator, system, debug, shelf, sync, and dictionary boards.
