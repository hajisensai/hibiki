...
- **[x] ① 已修复** - `_mobileControlsTheme` 取消移动端三点菜单和 `_showMobileMoreMenu` 分支，顶栏直接保留截图、字幕源、音轨和设置；窄屏底栏移除 10 秒后退/前进，只保留时间、上一句、播放、下一句、全屏。
- **[x] ② 已加自动化测试** - `hibiki/test/pages/video_mobile_controls_guard_test.dart` 守住移动端顶栏不再包含 `Icons.more_vert` / `_showMobileMoreMenu` / 宽度分支，并守住截图、字幕源、音轨、设置仍直接在顶栏；窄屏底栏不再包含 `Icons.replay_10` / `Icons.forward_10`，同时保留前后句、播放和全屏。
