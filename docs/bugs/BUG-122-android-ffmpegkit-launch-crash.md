...
- **备注**：
  ...
  - **Android ABI**：arm64-v8a + armeabi-v7a 均自编并打进 APK（覆盖几乎所有真机）。x86/x86_64（需 nasm，仅模拟器）暂缺——纯模拟器场景待补。
  - **iOS（已验证）**：同样自编 arthenica ffmpeg-kit 的 iOS `min` xcframeworks（Mac 上从零搭 autotools：m4/autoconf/automake/libtool 源码自编补齐 `autoreconf`，Xcode 16.4）。8 个 xcframework（ffmpegkit + 7 libav*，device arm64/arm64e + simulator arm64/x86_64，剥离 maccatalyst）vendored 到 `third_party/ffmpeg_kit_flutter/ios/Frameworks/`，podspec 改 `vendored_frameworks` + 系统库（替代已停服的 cocoapods pod）。**iPhone 16 Pro 模拟器实测**：最小 app 跑 `ffmpeg -version` rc=0（日志 `Loaded ffmpeg-kit-flutter-ios-min-arm64_simulator-6.0.3`）→ iOS 上自编 ffmpeg 真能跑。注：Hibiki 全量 iOS app 构建另有一个**与 ffmpeg 无关**的 `fluttertoast` 模块在新工具链报错（既有插件问题，pod install 已接受我们的 ffmpeg 集成、零 ffmpeg 报错）。
  - **体积**：vendored 自编原生库入库 ~76MB（Android AAR 20MB + iOS xcframeworks 56MB）——self-contained 代价；若嫌大可后续改 Git LFS 或构建期拉取。
