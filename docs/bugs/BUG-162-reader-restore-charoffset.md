...
- **[x] ① 已修复** — 持久化恢复改走精确字符偏移坐标，复用成熟路径，不破坏旧数据：
  - DB 阅读位置精确字符偏移用**单一列** `reader_positions.char_offset`（section 内绝对字符偏移恢复锚，
    schema v23→v24，`tables.dart` + `database.dart` `if (from < 24)`：ADD `char_offset` + DROP
    `ttu_char_offset`）。原 `ttu_char_offset` 是 sync 的 whole-book `exploredCharCount` **精确缓存**列，
    与 char_offset 是两套不同坐标系（外部 ッツ 口径·全书 vs WebView DOM 口径·章内）；因云同步当前无用户
    （用户决定），把 sync 对它的依赖拆掉（`sync_manager.dart` 改为只由 `(sectionIndex, normCharOffset)`
    经 `toExploredCharCount` 换算，精度退化为 normCharOffset 0-10000 分数，可接受），合并为单一精确列。
  ...
  - 提交：`a00fbb63c`（DB v24 初版）/ `39ae6a8ca`（保存端）/ `23c5da2f6`（恢复端+Dart 接线）/
    `f6dbccbaf`（合并为单列 char_offset、删 ttu_char_offset、拆 sync 缓存）。
