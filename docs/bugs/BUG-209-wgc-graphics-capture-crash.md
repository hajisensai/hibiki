...
- **[x] (1) 已修复** —— texture_bridge.cc 代际 retired 列表 + closed-flag 双保险（commit afaf8c95e）。Windows release 重编 √ Built hibiki.exe 通过；启动 smoke 无新 GraphicsCapture 崩溃。运行时完整复现（用户读书/查词 teardown 路径）待用户真机确认。
- **[x] (2) 已加自动化测试** —— 源码扫描守卫 hibiki/test/widgets/texture_bridge_stop_guard_test.dart（断言 teardown 顺序 Close+retire 不变量），绿。Windows 原生 WGC host 单测跑不了，故用源码守卫钉死 teardown 不变量。
