...
- **[x] ① 已修复** —— texture_bridge.cc 代际 retired 列表 + closed-flag 双保险（commit afaf8c95e）。Windows release 重编 √ Built hibiki.exe 通过；启动 smoke 无新 GraphicsCapture 崩溃。运行时完整复现（用户读书/查词 teardown 路径）待用户真机确认。
