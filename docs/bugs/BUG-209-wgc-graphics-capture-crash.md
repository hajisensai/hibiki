...
### 第十六修（TODO-479：handler 栈内 defer 失败不伪闭合，2026-06-17）

**复发证据**：用户 WGC 日志出现 `retire-defer-fail hr=0x8000001C`，随后又出现 `remove-before-close-fail defer_enqueue=0`。这说明第十五修在 FrameArrived handler 栈内触发退役时，尝试把 finalize 投递到 DispatcherQueue；但投递失败后仍调用 `FinalizeFramePoolLifetime(lifetime, false)`，进入“不允许 remove”的 finalize 分支，继续写 session/pool close 与 retire register。这种日志不是成功闭合，而是把无法离开 handler 的状态伪装成闭合，`remove-before-close-done` 与 `handler-release-done` 均不可证明。

**根因**：`RetireFramePoolLocked()` 的 handler 栈内路径违反第十五修自己的契约。handler 栈内只能做两件事：把 callback state 标记为 inactive/retiring，或成功排队到同一 DispatcherQueue 下一拍。只要排队失败，就仍处在当前 FrameArrived event 迭代/handler 调用栈内，不能同步 `remove_FrameArrived`、不能释放 handler、不能 Close session/pool、不能 `MarkRetired`。旧 pool/session/handler/token 应继续由 `WgcFramePoolLifetime` 与进程级 registry 保活，作为异常证据留到进程退出。

**第十六修实现**：
- **[x] ① 根因修复** —— `texture_bridge.cc/.h` 删除 `FinalizeFramePoolLifetime(..., allow_remove=false)` 旁路；handler 栈内 `TryEnqueue` 成功后才设置 `finalize_posted` 并写 `retire-defer-posted`，失败或 dispatcher 不可用时只写 `retire-defer-keepalive` 并返回，旧 `pool/session/handler/token` 保持存活，不写 `remove-before-close-done`、`session-close-done`、`pool-close-done`、`retire-register-done` 等成功事件。`FinalizeFramePoolLifetime` 只保留真正离开 handler 后的 remove-before-close 正常路径。
- **[x] ② 负向自动化守卫** —— 新增 `hibiki/test/widgets/wgc_capture_lifecycle_log_guard_test.dart`：用户失败日志样本必须判 invalid，成功样本必须按同一 pool/generation 顺序闭合 `remove-before-close-done -> handler-release-done -> session-close-done -> pool-close-done -> retire-register-done`；硬禁 `retire-defer-fail`、`remove-before-close-fail`、`remove-before-close-closed-unexpected`、`retire-remove-closed` 与 `defer_enqueue=0`。同步更新 `texture_bridge_stop_guard_test.dart` 与 `wgc_capture_logging_guard_test.dart`，禁止旧失败事件名回潮。

**验证边界**：源码守卫已证明 TODO-479 用户失败日志被拒、成功闭合日志被接受，并且 C++ 源不再含 `retire-defer-fail` / `defer_enqueue=0` 伪闭合路径。仍需 Windows 实机走原始路径（查词弹窗/WebView 开关/窗口 resize ≥10 轮）确认 `%LOCALAPPDATA%\Hibiki\wgc_capture.log` 零负向事件且 `%LOCALAPPDATA%\CrashDumps` 无新 `GraphicsCapture` dump。
