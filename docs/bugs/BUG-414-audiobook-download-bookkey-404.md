...
- **[x] ① 已修复** — `remote.part.dart:335` 改 `final String remoteBookKey = book.downloadId;`（与 EPUB 下载同源，消除不对称）；`sync_compare_dialog.dart:_downloadLiveAudiobookFor` 改为按 `title` 在 `listRemoteAudiobooks()` 清单里找该书的真实 `RemoteAudiobookInfo.bookKey` 下载（不再 sanitize 重算 + any 比对）。提交：d6ba8d5d9
- **[x] ② 已加自动化测试** —
