...
- **[x] ① 已修复** — 三处串联：
  ...
  - (c) `video_quick_settings_sheet.dart` 在 `_buildSeekSecondsRow` 后加 `_buildDoubleTapRow`（`AdaptiveSettingsPickerRow<int>` chips：关/3s/5s/10s/下一句），onChanged 走 `_commitAsb(copyWith)`。i18n 新增 key。提交：（见本轮 commit）
