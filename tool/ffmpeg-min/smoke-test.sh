...
run "$FIXTURE_FFMPEG" -hide_banner -loglevel error -y \
  ...
  -c:a aac -c:s mov_text -shortest "$WORK/h264-movtext.mp4"
...
run "$FIXTURE_FFMPEG" -hide_banner -loglevel error -y \
  ...
  -map 0:v -map 1:a -map 2:s \
  -c:v libx264 -preset ultrafast -pix_fmt yuv420p \
  -c:a libopus -c:s ass -shortest "$WORK/h264-ass.mkv"
...
run "$FFMPEG_MIN" -hide_banner -loglevel error -y \
  -i "$WORK/h264-movtext.mp4" -map 0:s:0 "$WORK/movtext.srt"
assert_nonempty "$WORK/movtext.srt"

run "$FFMPEG_MIN" -hide_banner -loglevel error -y \
  -i "$WORK/h264-ass.mkv" -map 0:s:0 "$WORK/embedded.ass"
assert_nonempty "$WORK/embedded.ass"

echo "[ffmpeg-min-smoke] exporting cue GIF and frame"
run "$FFMPEG_MIN" -hide_banner -loglevel error -y \
  -ss 0.100 -t 1.000 -i "$WORK/h264-movtext.mp4" -an \
  -filter_complex \
...
for output in \
  "$WORK/cue.gif" \
  "$WORK/frame.jpg" \
  "$WORK/h264-movtext.mp4.aac" \
  "$WORK/h264-ass.mkv.aac" \
  "$WORK/tone.ac3.aac" \
  "$WORK/tone.eac3.aac" \
  "$WORK/tone.wma.aac" \
  "$WORK/tone.wav.aac" \
