...
/// TODO-309, BUG-263 / BUG-264 / BUG-265). The page-level wiring lives in the
...
void main() {
  ...
  group('TODO-278/BUG-263 list-row word lookup wiring', () {
    test('VideoSubtitleJumpPanel is given onLookupCue', () {
      expect(
        ...
        reason: 'jump panel must receive onLookupCue so list rows can look up '
            'words (revert -> rows cannot look up, BUG-263)',
      );
    });
  ...
  group('TODO-301/BUG-264 favorite marker wiring', () {
    ...
    test('favorite cache is refreshed on video open (after warm-popup seed)',
      ...
      expect(
        ...
        reason:
            ...
            'before the subtitle list is ever opened (BUG-264)',
      );
    });
  });
}
