...
/// a real player.
///
/// - BUG-124: 点击画面不暂停（media_kit 桌面 `playAndPauseOnTap` 默认 false）。
/// - BUG-125: 导入字幕后键盘失灵（加载遮罩夺焦后未归还）。
/// - BUG-126: 退出后导入字幕丢（播放列表恢复不按路径加载 app 文档目录里的导入文件）。
/// - BUG-127: 视频画面拖入字幕无反应（窗口模式缺页级拖放目标）。
void main() {
  final String src =
  ...
  test('BUG-126: 恢复字幕源时对导入的外挂文件按路径直接加载', () {
    final String body = region(
  ...
  test('BUG-127: 视频页有页级拖放目标 + 导入去重防护', () {
    // 页级拖放目标（窗口模式可靠收拖放；内层那个供全屏用）。
    expect(src.contains('Widget _pageDropTarget('), isTrue,
        reason: '窗口模式需在页面顶层挂拖放目标，内层 media_kit controls 里的实测无反应（BUG-127）');
