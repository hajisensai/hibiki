...
- `TtuMigration` / `TtuMigrationServer` -- 旧 TTU 数据迁移（只读遗留）。
...
- `TextToEpub` -- 文本转 EPUB。
- `TtuIdbReader` / `ReadingStatisticIdbReader` -- 旧数据读取。
...
- `AppModel` -- 全局应用状态（Riverpod `appProvider`，3146 行），管理初始化、主题、语言、词典、导航。
  - **初始化流程** (`initialise()`): PackageInfo → 目录创建 → Drift DB 打开 → 偏好加载 → Profile 确保 → 词典缓存 → 媒体历史 → 主题调色板 → 语言/格式/增强/快捷操作注册 → TTU 迁移(仅旧版本) → 搜索预热
  ...
  - **弹窗词典入口** (`initialiseForDictionaryPopup()`): 精简初始化路径（跳过 TTU 迁移、MediaSource、QuickAction）
...
- `reader_hibiki_page.dart` (4088 行) -- 核心阅读器页面：
...
- **dependency_overrides**：`file_picker` -> 本地 fork，`flutter_inappwebview_windows` -> 本地 fork

## 数据模型

数据模型全部定义在 `hibiki_core`（21 张 Drift 表），本模块仅消费。
...
- `assets/ttu-ebook-reader/` -- 旧 TTU 迁移用 HTML。
