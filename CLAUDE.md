...
- 旧 TTU IndexedDB 迁移代码（`TtuMigrationServer` / `TtuIdbReader` / `TtuMigration` / `assets/ttu-ebook-reader`）已删除，不再支持从历史 IndexedDB 数据自动迁移。当前阅读器问题不要去 `D:\ttu-fork` 修。
...
- Android 资源、manifest、Gradle、权限、通知、前台服务或打包行为改动：还要运行：
  ...
  ```
- 修改当前阅读器 WebView、JS、CSS、资源拦截或分页逻辑时，只在 Hibiki 侧验证 Hoshi 阅读器路径。
