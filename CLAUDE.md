...
集成测试只跑在**模拟器**上（`ci/integration-test.sh` 会忽略物理真机，只用 `emulator-<port>`）。无模拟器在线时会自动从 AVD 启一台。
...
**关键约束：**
- ADB 脚本（`ci/*.sh`）**不得**尝试向 Flutter 文本框输入 CJK——`input text` 在 Android 上不支持 Unicode，`settext.jar` 找不到 Flutter 的 EditText。CJK 文字输入只能通过 `tester.enterText()` 在 Flutter 集成测试里完成。
...
集成测试的唯一入口是 `ci/integration-test.sh`：自动选/启一台**模拟器**（物理真机会被忽略——只认 `emulator-<port>` 序列号），构建一次 debug APK，自动 provision 所有前置（AnkiDroid 安装+建 collection+授权、字典 zip 推 `/sdcard`），再遍历全部 `integration_test/*_test.dart` 目标逐个 `flutter drive`，最后打印分类 PASS/SKIP/FAIL 汇总（任一失败退出非零）。无需真机、无需手动 `flutter drive`、无需手动 adb 推送或点击。

```bash
bash ci/integration-test.sh                      # 起/选模拟器，构建，provision，跑全部 15 个目标
...
每个目标的日志落在 `.codex-test/itest-logs/<target>.log`。书库依赖类测试（reader_dictionary / reader_keyboard / regression）通过 `integration_test/helpers/library_fixture.dart` 在测试内自带 fixture（`seedReaderBook` 用 `EpubGenerator`+`EpubImporter` 程序化导入合成 EPUB，`seedDictionary` 导入 runner 推到 `/sdcard/Download/test_dict.zip` 的字典），因此在全新安装上也无需手动导入。

> 说明：旧文档里的 `.codex-test/tools/test-flows.ps1`（`-Flow push-fixtures/db-verify/all`）已不存在；推素材的独立脚本是 `ci/emulator-test.sh`，AnkiDroid 单独链路是 `ci/anki-integration-test.sh`（其 provision 逻辑被 `ci/integration-test.sh` 经 `ci/lib/provision-ankidroid.sh` 复用）。DB 状态直查仍用 `run-as app.hibiki.reader sqlite3 files/hibiki.db`。
...
iOS/macOS 构建走局域网内一台远程 Mac。连接信息（SSH 主机/端口）、CocoaPods 在 ruby 2.6 上的钉版清单、构建/运行/热重载流程放在本机未入库文件 `AGENTS.local.md`，不上传 git。需要时直接读取 `AGENTS.local.md`。
...
- 2026-05-30: 新增「iOS 模拟器构建（远程 Mac）」章节 -- 远程 Mac 构建链路、git-over-SSH 同步、CocoaPods 在 ruby 2.6 上的钉版清单、`third_party/` + `dependency_overrides`、构建/运行/热重载流程、x86_64-Rosetta 与免 EXCLUDED_ARCHS 说明。（私密连接细节后于 2026-06-02 移至本机未入库 `AGENTS.local.md`。）
