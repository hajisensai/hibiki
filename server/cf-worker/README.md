# Hibiki 报错日志接收 Worker（Cloudflare Worker + KV）

CF 边缘上的报错日志接收端点，**零服务器、免备案、免费额度内即可用**。App 上传 JSON 日志，Worker 把它当字符串存进 Workers KV；管理员经浏览器用 HTTP Basic Auth 查看列表与单条日志。
...
- **白名单 id**：读取前过 `^\d{8}-\d{6}-[a-z]+-[a-z0-9]{6}\.txt$` 正则；KV 无文件路径，路径穿越天然不存在，但仍做白名单兜底。
...
# 创建 KV namespace，把返回的 id 填进 wrangler.toml 的 id 字段
...
vitest 在 Node 下直接 `import worker from '../src/worker.js'` 调 `worker.fetch(request, env)`，KV 用假对象注入，覆盖纯函数与路由（含路径穿越、XSS 惰化、fail-closed、413 等）。
